...
typedef enum
{
  ...
  APP_SEQUENCE_STATE_HOMING,         // 전원 초기화 또는 RESET으로 초기 위치 보정 중인 상태
  APP_SEQUENCE_STATE_IDLE_HOME,      // 초기 위치 보정이 끝나고 START를 기다리는 상태
  ...
  APP_SEQUENCE_STATE_MOVING_TO_HOME, // STOP 또는 FOOT 반복 중 스텝모터가 시작 위치로 복귀 중인 상태
  APP_SEQUENCE_STATE_ERROR,          // 명령 실패 또는 센서 미감지 등으로 에러가 발생한 상태
} app_sequence_state_t;

// 장비 시퀀스 초기화 후 초기 위치 보정을 수행
